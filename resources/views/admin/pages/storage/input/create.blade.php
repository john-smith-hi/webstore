...
@endsection
@section('content')
<form method="POST" action="{{ route('admin.storage.input.store') }}">
    @csrf
    <div class="form-row">
      ...
      <div class="form-group col-md-6">
        <label>Số lượng nhập</label><span style="color: red">(*)</span>
      ...
      <div class="form-group col-md-6">
        <label>Giá nhập</label><span style="color: red">(*)</span>
        <input name="price" type="number" class="form-control" placeholder="Giá nhập" required value="@if(!empty(old('price'))){{old('price')}} @else{!!'0'!!}@endif">
        @error('price')
            <span style="color: red">{{$message}}</span>
        @enderror
      </div>
