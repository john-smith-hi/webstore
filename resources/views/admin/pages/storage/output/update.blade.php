...
@endsection
@section('content')
<form method="POST" action="{{ route('admin.storage.output.edit',['id'=>$outputStorage]) }}">
    @csrf
    @php
      $product = $Product->find($outputStorage->product_id);
      $search_product_name = $product->name;
      $product_id = $outputStorage->product_id;
      $origin_price = $product->price;
      $quantity = $outputStorage->quantity;
      $price = $outputStorage->price;
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-6">
        ...
        <input name="price" type="number" class="form-control" placeholder="Giá xuất" required value="{{$price}}">
        @error('origin_price')
            <span style="color: red">{{$message}}</span>
        @enderror
      </div>
